...
describe Chef::Util::PathHelper do
  ...
  describe "valid_path?" do
    context "on windows" do
      ...
      it "returns true if the path passes the tests" do
        expect(path_helper.valid_path?("Newline!\n")).to be_true
      end

      it "returns false if the path has invalid characters" do
        path_helper.stub(:printable?).and_return(false)
        expect(path_helper.valid_path?("Newline!\n")).to be_false
      end

      it "returns false if the path exceeds MAX_LENGTH" do
